...
namespace Platform.DAAS.OData.HTTP
{
    public class ResourceRouter : IResourceRouter
    {
        ...
        ITracer tracer;
        IExHandler exHandler;

        ITracer Tracer { get { return this.tracer; } set { this.tracer = value; } }
        ...
        private X509Certificate getCertificate(string certificatePath, string password)
        {
            ...
            if (this.EnableTracing && (this.tracer != null))
            {
                this.tracer.Trace(new object[] { "Certificate information:", certificatePath, returnValue.Issuer, returnValue.Subject, returnValue.GetSerialNumberString(), returnValue.GetRawCertDataString(), returnValue.GetPublicKeyString() }, this.TraceSourceName);
            }      
        ...
        private X509Certificate getCertificate(byte[] certificateData, string password)
        {
            ...
            if (this.EnableTracing && (this.tracer != null))
            {
                this.tracer.Trace(new object[] { "Certificate information:", certificateData, returnValue.Issuer, returnValue.Subject, returnValue.GetSerialNumberString(), returnValue.GetRawCertDataString(), returnValue.GetPublicKeyString() }, this.TraceSourceName);
            }
        ...
        private string createSyncRequest(string uri, string message, string method, Authentication authentication, Dictionary<string, string> headers)
        {
            ...
            try
            {
                ...
                if (this.EnableTracing && (this.tracer != null))
                {
                    this.tracer.Trace(new object[] { "Request information:", request.Headers, request.UserAgent, request.RequestUri, message, request.Method, request.Host, request.Date }, this.TraceSourceName);
                }
            }
            catch (Exception ex)
            {
                ...
                if (this.EnableTracing && (this.tracer != null))
                {
                    this.tracer.Trace(new object[] { "Exception information:", ex.ToString(), message, uri, method }, this.TraceSourceName);
                }

                this.exHandler.HandleException(ex);
            ...
            if (response != null)
            {
                ...
                if (this.EnableTracing && (this.tracer != null))
                {
                    this.tracer.Trace(new object[] { "Response information:", response.Headers, response.ResponseUri, returnValue }, this.TraceSourceName);
                }
            }
        ...
        private void getRequestStreamAsyncCallback(IAsyncResult result)
        {
            ...
            if ((states != null) && (states.Length > 0))
            {
                ...
                try
                {
                    ...
                    if (this.EnableTracing && (this.tracer != null))
                    {
                        this.tracer.Trace(new object[] { "Request information:", request.Headers, request.UserAgent, request.RequestUri, data, request.Method }, this.TraceSourceName);
                    }
                }
                catch (Exception ex)
                {
                    if (this.EnableTracing && (this.tracer != null))
                    {
                        this.tracer.Trace(new object[] { "Exception information", ex.ToString(), request.Headers, request.UserAgent, request.RequestUri, data, request.Method }, this.TraceSourceName);
                    }
        ...
        private void getResponseAsyncCallback(IAsyncResult result)
        {
            ...
            if ((states != null) && (states.Length > 0))
            {
                ...
                try
                {
                    ...
                    if (this.EnableTracing && (this.tracer != null))
                    {
                        this.tracer.Trace(new object[] { "Request information:", request.Headers, request.UserAgent, request.RequestUri, request.Method }, this.TraceSourceName);
                    }
                }
                catch (Exception ex)
                {
                    if (this.EnableTracing && (this.tracer != null))
                    {
                        this.tracer.Trace(new object[] { "Exception information", ex.ToString(), request.Headers, request.UserAgent, request.RequestUri, request.Method }, this.TraceSourceName);
                    }
