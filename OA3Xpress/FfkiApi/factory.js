...
var hqaOutputRepository = config.get("app.hqa-output-repository");
...
var hqaOutputUploader = multer({
    storage: multer.diskStorage({
        destination: function (req, file, cb) {
            cb(null, hqaOutputRepository);
        },
        filename: function (req, file, cb) {
            cb(null, Date.now().toString() + "_" + file.originalname);
        }
...
app.post("/hqa/output/", function (req, res) {

    hqaOutputUploader(req, res, function (err) {
        if (err) {
            console.log(err);
            res.end(err);
        }
