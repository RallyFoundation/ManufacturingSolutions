...
$OA3ToolConfigurationFilePath = $RootDir + "\OA3Tool\x86\OA3Tool-ServerBased.cfg";

if($Architecture.ToLower() -eq "amd64")
{
   $OA3ToolPath = $RootDir + "\OA3Tool\amd64\oa3tool.exe";

   $OA3ToolConfigurationFilePath = $RootDir + "\OA3Tool\amd64\OA3Tool-ServerBased.cfg";
}
...
#And appends OHR data, serial number, BusinessID to OA3Tool configuration file (if there is not any for each):
try
{
...
try
{
    ...
    $Message;
    $Message | Out-File -FilePath ($LogPath + "\production-log.log") -Append;
    Start-Process -FilePath ($RootDir + "\\InjectionTools\Flash_afuwin\AFUWin.exe")  -NoNewWindow -ArgumentList @([System.String]::Format("/A{0}", $OA3OutputBinFilePath)) -Wait | Out-File -FilePath ($LogPath + "\production-log.log") –Append;
}
catch [System.Exception]
{
    $Message = $Error[0].Exception;
