param([System.String]$RootDir, [System.String]$Architecture = "amd64", [System.String]$NIC, [System.String]$IsShowingBarcode = "false", [System.String]$MESServerAddress, [System.String]$MESServerAuth = "MES:M(S@OMSG.msft");
...
$UriAddTrans = "/Transaction/New/";

$RequestAuthHeader = "MES:M(S@OMSG.msft";

if([System.String]::IsNullOrEmpty($MESServerAuth) -eq $false)
{
   $RequestAuthHeader = $MESServerAuth;
}

$RequestHeaders = @{Authorization = $RequestAuthHeader};

[xml]$CurrentTransaction = '<ProductKeyIDSerialNumberPairs xmlns="http://schemas.datacontract.org/2004/07/MES.Persistency" xmlns:i="http://www.w3.org/2001/XMLSchema-instance">
...
{
   try
   {      $RequestUri = $ServerAddress + $ServicePoint + $UriAddTrans;      
      $CurrentTransaction.ProductKeyIDSerialNumberPairs.TransactionID = $TransactionID;

      $CurrentTransaction.ProductKeyIDSerialNumberPairs.PairID = $TransactionID;

      $CurrentTransaction.ProductKeyIDSerialNumberPairs.ProductKeyID = $ProductKeyID

      $CurrentTransaction.ProductKeyIDSerialNumberPairs.SerialNumber = $SerialNumber;

      $CurrentTransaction.ProductKeyIDSerialNumberPairs.CreationTime = [System.DateTime]::Now.ToString("MM/dd/yyyy HH:mm:ss");      [Microsoft.PowerShell.Commands.WebResponseObject]$ResultValue = Invoke-WebRequest -Uri $RequestUri -Method Post -ContentType "text/xml; charset=utf-8" -Headers $RequestHeaders -Body $Current;      $ResultValue.Content;      $ResultValue.Content | Out-File -FilePath ($LogPath + "\production-log.log") -Append;      [xml]$ResultXML =  New-Object System.Xml.XmlDocument;      $ResultXML.LoadXml($ResultValue.Content.ToString());      $ResultXML.string.'#text';
