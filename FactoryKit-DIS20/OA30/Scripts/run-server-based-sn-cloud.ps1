param([System.String]$RootDir, [System.String]$Architecture = "amd64", [System.String]$MESServerAddress);
...
if([System.String]::IsNullOrEmpty($MESServerAddress) -eq $true)
{
   $Host.UI.RawUI.BackgroundColor = "Red";
   $Host.UI.RawUI.ForegroundColor = "Yellow";
   Write-Host -Object "MES Cloud server address not supplied! Please supply one first.";
...
$ServerAddress = "http://mes-server-01:8919";

$ServerAddress = $MESServerAddress;

if($ServerAddress.EndsWith("/") -eq $true)
{
   $ServerAddress = $ServerAddress.Substring(0, ($ServerAddress.Length - 1));
}

$ServicePoint = "/Services/MESService.svc";

$UriGetTrans = "/Transaction/First/";

$UriSetTrans = "/Transaction/{0}/{1}/{2}/";

$RequestAuthHeader = "MES:M(S@OMSG.msft";

$RequestHeaders = @{Authorization = $RequestAuthHeader};
...
#Reads out serial number from MES Cloud
try
{
   $Message = [System.String]::Format("Reading serial number from MES Cloud..., {0}", [System.DateTime]::Now);
   $Message;
   $Message | Out-File -FilePath ($LogPath + "\production-log.log") -Append;

   $RequestUri = $ServerAddress + $ServicePoint + $UriGetTrans;

   [Microsoft.PowerShell.Commands.WebResponseObject]$ResultValue = Invoke-WebRequest -Uri $RequestUri -Method Get -ContentType "text/xml; charset=utf-8" -Headers $RequestHeaders;     $ResultValue.Content;   $ResultValue.Content | Out-File -FilePath ($LogPath + "\production-log.log") -Append;   [xml]$ResultXML =  New-Object System.Xml.XmlDocument;   $ResultXML.LoadXml($ResultValue.Content.ToString());   $CurrentTransaction = $ResultXML.ProductKeyIDSerialNumberPairs;   if(($CurrentTransaction -eq $null) -or ([System.String]::IsNullOrEmpty($CurrentTransaction.TransactionID) -eq $true))   {      $Host.UI.RawUI.BackgroundColor = "Red";
      $Host.UI.RawUI.ForegroundColor = "Yellow";      Write-Host -Object "Could NOT get a valid transation from MES Cloud!";
      Read-Host -Prompt "Press any key to exit...";
      exit;   }   $TransactionID = $CurrentTransaction.TransactionID;   $SerialNumber = $CurrentTransaction.SerialNumber;
   ...
   if([System.String]::IsNullOrEmpty($SerialNumber) -eq $true)
   {
       $Host.UI.RawUI.BackgroundColor = "Red";
       $Host.UI.RawUI.ForegroundColor = "Yellow";
       Write-Host -Object "Serial number from MES Cloud is empty!";
       Read-Host -Prompt "Press any key to exit...";
       exit;
   }
}
...
{  
    ...
    $Host.UI.RawUI.BackgroundColor = "Red";
    $Host.UI.RawUI.ForegroundColor = "Yellow";
    Write-Host -Object "Error(s) occurred reading serial number from MES Cloud!";
    Read-Host -Prompt "Press any key to exit...";
    exit;
}
...
##, as well as the MES Cloud
#if($MatchCount -eq 2)
if([System.String]::IsNullOrEmpty($SerialNumber) -eq $false)
{
   try
   {
      #$UriSetTrans =  [System.String]::Format("/Transaction/{0}/{1}/{2}/", $TransactionID, $SerialNumber, $ProductKeyID);      $UriSetTrans =  [System.String]::Format("/Bind/{0}/{1}/", $SerialNumber, $ProductKeyID);      $RequestUri = $ServerAddress + $ServicePoint + $UriSetTrans;      [Microsoft.PowerShell.Commands.WebResponseObject]$ResultValue = Invoke-WebRequest -Uri $RequestUri -Method Post -ContentType "text/xml; charset=utf-8" -Headers $RequestHeaders;      $ResultValue.Content;      $ResultValue.Content | Out-File -FilePath ($LogPath + "\production-log.log") -Append;      [xml]$ResultXML =  New-Object System.Xml.XmlDocument;      $ResultXML.LoadXml($ResultValue.Content.ToString());      $ResultXML.string.'#text';

      $ResultXML.string.'#text' | Out-File -FilePath ($LogPath + "\production-log.log") -Append;
