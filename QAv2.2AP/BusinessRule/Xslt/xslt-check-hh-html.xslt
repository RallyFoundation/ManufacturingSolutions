...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:msxsl="urn:schemas-microsoft-com:xslt" exclude-result-prefixes="msxsl" xmlns:hhvxslx="HHValidation.XsltExt">
    ...
    <html lang="en" xmlns="http://www.w3.org/1999/xhtml">
      ...
      <body>
        ...
        <ul>
          ...
          <li>
            <xsl:call-template name="Autopilot"/>
          </li>
        </ul>
      </body>
    </html>
  ...
  <xsl:template name="Autopilot">
    ...
    <xsl:variable name="SmbiosSystemSerialNumber" select="/TestItems/SmbiosSystemSerialNumber"/>
    <xsl:variable name="SmbiosUuid" select="/TestItems/SmbiosUuid"/>
    <!--<xsl:variable name="SmbiosRamErrorCorrection" select="/TestItems/SmbiosRamErrorCorrection"/>
    <xsl:variable name="SmbiosRamSlots" select="/TestItems/SmbiosRamSlots"/>
    <xsl:variable name="SmbiosRamArrayCount" select="/TestItems/SmbiosRamArrayCount"/>
    <xsl:variable name="SmbiosRamMaximumCapacity" select="/TestItems/SmbiosRamMaximumCapacity"/>-->
    <xsl:variable name="TPMEkPub" select="/TestItems/TPMEkPub"/>
    <xsl:variable name="DiskSerialNumber" select="/TestItems/DiskSerialNumber"/>
    <xsl:variable name="ProductKeyID" select="/TestItems/ProductKeyID"/>
    <xsl:variable name="OSType" select="/TestItems/OSType"/>
    <xsl:variable name="MacAddress" select="/TestItems/NIC"/>
    <xsl:variable name="IsPassed" select="($TPMEkPub/Result = 'Passed') and ($DiskSerialNumber/Result = 'Passed') and ($SmbiosSystemManufacturer/Result = 'Passed') and ($ProductKeyID/Result = 'Passed') and ($OSType/Result = 'Passed') and ($MacAddress/Result = 'Passed') and ($SmbiosSystemProductName/Result = 'Passed') and ($SmbiosSystemSerialNumber/Result = 'Passed') and ($SmbiosUuid/Result = 'Passed')"/>
    <p style="font-weight:bolder;font-size:x-large;text-decoration:underline;font-family:Arial">
      Autopilot Fields:
      <xsl:value-of select="'&amp;nbsp;'" disable-output-escaping="yes"/>
      <xsl:if test="$IsPassed = true()">Passed</xsl:if>
    ...
    <ul>
      <xsl:if test="$DiskSerialNumber != ''">
        <li>
          <p style="font-weight:bolder;text-decoration:underline;font-style:italic">DiskSerialNumber: </p>
          <table xmlns="http://www.w3.org/1999/xhtml">
            <tr>
              <td>Min Value: </td>
              <td>
                <xsl:value-of select="hhvxslx:GetHtmlSpacedString($DiskSerialNumber/Min)" disable-output-escaping="yes"/>
              </td>
            </tr>
            <tr>
              <td>Max Value: </td>
              <td>
                <xsl:value-of select="hhvxslx:GetHtmlSpacedString($DiskSerialNumber/Max)" disable-output-escaping="yes"/>
              </td>
            </tr>
            <tr>
              <td>Test Value: </td>
              <td style="background-color:yellow">
                <xsl:value-of select="hhvxslx:GetHtmlSpacedString($DiskSerialNumber/Value)" disable-output-escaping="yes"/>
                (Length: <xsl:value-of select="string-length($DiskSerialNumber/Value)"/> characters)
              </td>
            </tr>
            <tr>
              <td>Result: </td>
              <td>
                <xsl:if test="$DiskSerialNumber/Result = 'Passed'">
                  <xsl:attribute name="style">background-color:forestgreen</xsl:attribute>
                </xsl:if>
                <xsl:if test="$DiskSerialNumber/Result = 'Failed'">
                  <xsl:attribute name="style">background-color:red</xsl:attribute>
                </xsl:if>
                <xsl:value-of select="$DiskSerialNumber/Result"/>
              </td>
            </tr>
            <tr>
              <td>Detail: </td>
              <td>
                <xsl:value-of select="$DiskSerialNumber/Detail"/>
              </td>
            </tr>
          </table>
        </li>
      ...
      <xsl:if test="$TPMEkPub != ''">
        <li>
          <p style="font-weight:bolder;text-decoration:underline;font-style:italic">TPMEkPub: </p>
          <table xmlns="http://www.w3.org/1999/xhtml">
            <tr>
              <td>Min Value: </td>
              <td>
                <xsl:value-of select="hhvxslx:GetHtmlSpacedString($TPMEkPub/Min)" disable-output-escaping="yes"/>
              </td>
            </tr>
            <tr>
              <td>Max Value: </td>
              <td>
                <xsl:value-of select="hhvxslx:GetHtmlSpacedString($TPMEkPub/Max)" disable-output-escaping="yes"/>
              </td>
            </tr>
            <tr>
              <td>Test Value: </td>
              <td style="background-color:yellow">
                <xsl:value-of select="hhvxslx:GetHtmlSpacedString($TPMEkPub/Value)" disable-output-escaping="yes"/>
              </td>
            </tr>
            <tr>
              <td>Result: </td>
              <td>
                <xsl:if test="$TPMEkPub/Result = 'Passed'">
                  <xsl:attribute name="style">background-color:forestgreen</xsl:attribute>
                </xsl:if>
                <xsl:if test="$TPMEkPub/Result = 'Failed'">
                  <xsl:attribute name="style">background-color:red</xsl:attribute>
                </xsl:if>
                <xsl:value-of select="$TPMEkPub/Result"/>
              </td>
            </tr>
            <tr>
              <td>Detail: </td>
              <td>
                <!--<xsl:value-of select="$TPMEkPub/Detail"/>-->
                
                <xsl:for-each select="$TPMEkPub/Detail/child::*[starts-with(name(), 'TPMEkPub_')]">
                  <span>
                    <xsl:value-of select="name()"/>
                    <xsl:value-of select="':'"/>
                    <xsl:value-of select="'&amp;nbsp;'" disable-output-escaping="yes"/>
