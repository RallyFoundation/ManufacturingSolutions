...
namespace QA.Reducer
{
    public class ValidationResultXmlReducer : IReducer
    {
        public object Reduce(object Pairs, object Data)
        {
            ...
            IDictionary<string, string> tpmInfo = null;
                ...
                using (StreamWriter writer = new StreamWriter(stream, Encoding.UTF8) { AutoFlush = true })
                {
                    ...
                    foreach (string fieldName in resultSummary.Keys.Where(k => ((k != "ToolBuild") && (k != "ToolVersion"))))
                    {
                        ...
                        for (int i = 0; i < resultObjects[fieldName].Count; i++)
                        {
                            ...
                            if (result.FieldName == "TPMEkPub")
                            {
                                if ((result.FieldValue != null) && (result.FieldValue is IDictionary<string, string>))
                                {
                                    tpmInfo = (result.FieldValue as IDictionary<string, string>);

                                    foreach (string key in tpmInfo.Keys)
                                    {
                                        xmlWriter.WriteElementString(key, tpmInfo[key]);
                                    }
                                }
                            }
                        }
