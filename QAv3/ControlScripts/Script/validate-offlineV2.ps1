...
{
	$PromptResult = $Host.UI.Prompt("OA3Tool /Report XML Required", "Please specify the full path to the OA3Tool /Report XML result file:", "Path");

    $ReportFilePath = $PromptResult["Path"]; #$Host.UI.ReadLine(); #Get-InputPath -Title "Specify OA3Tool /Report File" -Message "Click 'Browse' to locate the output file from OA3Tool.exe /Report:" -ErrorMessage "Please specify the location to the output file from OA3Tool.exe /Report!" -AbortOnCancel $true -UseShortPath $false;
